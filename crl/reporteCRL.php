...
switch ($action) {
    ...
    case 'rptCA':
        ...
        $valores = [];
        for ($j = 0; $j < count($tiendas); $j++) {
            array_push($valores, array($mes.'/'.$anio, $tiendas[$j][0], '0.00', '0.00'));
        }
        $listar = rptCAr($mes, $anio);
        $listab = rptCAb($mes, $anio);
        
        for ($j = 0; $j < count($valores); $j++) {
            for ($i = 0; $i < count($listar); $i++) {
                if ($listar[$i][1] == $valores[$j][1]) {
                    $valores[$j][2] = $listar[$i][2];
                }
            }
            for ($i = 0; $i < count($listab); $i++) {
                if ($listab[$i][1] == $valores[$j][1]) {
                    $valores[$j][3] = $listab[$i][2];
                }
            }
        }
